...
def TableSimulation():
    ...
    sg.popup_quick_message('Hang on for a moment, this will take a bit to create....',
                           auto_close=True, non_blocking=True, font='Default 18')
    ...
    columm_layout = [[sg.Text(str(i), size=(4, 1), justification='right')] + [sg.InputText(size=(10, 1), pad=(
        1, 1), border_width=0, justification='right', key=(i, j)) for j in range(MAX_COL)] for i in range(MAX_ROWS)]
    ...
    layout = [[sg.Menu(menu_def)],
              ...
              [sg.Text('Type in a row, column and value. The form will update the values in realtime as you type'),
               sg.Input(key='inputrow', justification='right',
                        size=(8, 1), pad=(1, 1)),
               sg.Input(key='inputcol', size=(8, 1),
                        pad=(1, 1), justification='right'),
